...
/* ─── Static data ─── */
const STATS = [
  { label: "Members",  value: 25, suffix: "+", gradient: "from-blue-500 via-blue-500 to-indigo-600",      glow: "rgba(99,102,241,0.5)",   floatDelay: 0   },
  { label: "Meetings", value: 4,  suffix: "",  gradient: "from-emerald-400 via-teal-500 to-cyan-500",     glow: "rgba(20,184,166,0.5)",   floatDelay: 0.7 },
  { label: "Projects", value: 0,  suffix: "",  gradient: "from-violet-500 via-purple-500 to-fuchsia-500", glow: "rgba(168,85,247,0.5)",   floatDelay: 1.4 },
];
    ...
    >
      ...
      <Tabs defaultValue="overview" className="flex flex-col flex-1 z-10 relative">
        ...
        <header className="sticky top-0 z-50 w-full"
          style={{ background: "rgba(11,17,33,0.8)", backdropFilter: "blur(24px) saturate(180%)", borderBottom: "1px solid rgba(255,255,255,0.07)", boxShadow: "0 1px 40px rgba(0,0,0,0.4)" }}
        >
          <div className="max-w-5xl mx-auto px-5 h-16 flex items-center justify-between">
            <div className="flex items-center gap-2.5">
              <div className="p-1 rounded-xl" style={{ background: "rgba(255,255,255,0.07)", border: "1px solid rgba(255,255,255,0.1)" }}>
                <img src={`${import.meta.env.BASE_URL}lahs-logo.png`} alt="Logo" className="w-8 h-8 object-contain" />
              </div>
              <span className="font-black text-base tracking-tight">
                <span style={{ background: "linear-gradient(90deg,#60a5fa,#34d399)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>Game Dev</span>
                <span className="text-slate-200"> Club</span>
              </span>
            </div>
            ...
            >
              {[{ value: "overview", label: "Overview" }, { value: "about", label: "About Us" }, { value: "projects", label: "Projects" }].map(({ value, label }) => (
                <TabsTrigger key={value} value={value}
                  className="px-4 h-8 rounded-[10px] text-sm font-semibold text-slate-400 transition-all duration-200 data-[state=active]:bg-white/10 data-[state=active]:text-blue-300 data-[state=active]:shadow-sm"
                >
                  {label}
                </TabsTrigger>
              ))}
        ...
        <main className="flex-1 w-full max-w-5xl mx-auto px-5 pt-14 pb-20">
                ...
                return (
                  <motion.div key={label} variants={FADE_UP} className="flex flex-col items-center gap-3">
                    <div className="relative">
                      <div className="absolute inset-0 rounded-full blur-xl scale-110 opacity-80" style={{ background: `radial-gradient(circle, ${glow}, transparent)` }} />
                      <motion.div
                        ...
                        style={{ boxShadow: `0 20px 56px ${glow}` }}
                      >
              ...
              >
                ...
                <div className="flex flex-wrap gap-3">
                  <Button size="lg" asChild
                    className="h-11 px-7 rounded-2xl font-bold text-white border-0 hover:scale-105 active:scale-100 transition-all"
                  ...
                  <Button size="lg" variant="outline" asChild
                    className="h-11 px-6 rounded-2xl font-semibold hover:scale-105 active:scale-100 transition-all"
          ...
          <TabsContent value="about" className="mt-0 outline-none">
            <motion.div initial="hidden" animate="visible" variants={STAGGER} className="space-y-10">
              <motion.div variants={FADE_UP} className="text-center">
                ...
                <h2 className="text-3xl md:text-4xl font-black text-white tracking-tight">Leadership</h2>
              </motion.div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
                {MEMBERS.map((m, i) => (
                  ...
                  >
                    <div className="relative">
                      <div className="absolute inset-0 rounded-full blur-lg scale-125 opacity-50 group-hover:opacity-90 transition-opacity"
                    ...
                    <div>
                      ...
                      <p className="text-slate-100 font-bold">{m.name}</p>
                    </div>
                  </motion.div>
                ))}
              </div>
              ...
              <motion.div variants={FADE_UP}>
                ...
                >
                  <div>
                    <p className="text-xs font-bold uppercase tracking-widest text-slate-500 mb-1">General Members</p>
                    <p className="text-slate-300 font-medium">A growing community of developers, designers, and artists.</p>
                  </div>
                  <div className="flex items-center gap-3 shrink-0">
                    <div className="flex -space-x-2">
                      {["#3b82f6","#10b981","#8b5cf6","#f59e0b","#ef4444"].map((c,i) => (
                        <div key={i} className="w-8 h-8 rounded-full border-2 border-slate-800" style={{ background: c }} />
                      ))}
                    </div>
                    <span className="text-2xl font-black text-white">
                      {liveMembers !== null ? liveMembers : "25+"} <span className="text-slate-500 text-sm font-semibold">active</span>
                    </span>
                  </div>
                </div>
              </motion.div>
          ...
          <TabsContent value="projects" className="mt-0 outline-none">
            ...
            >
              ...
              <div>
                <h3 className="text-xl font-black text-white mb-1">Projects coming soon</h3>
                <p className="text-slate-500 text-sm max-w-xs">Our first batch of games is in the works. Check back soon!</p>
              </div>
            </motion.div>
          </TabsContent>
